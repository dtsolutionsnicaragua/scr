...
import componente.SincronizarComboMarca;
...
public class FrmModelo extends JPanel {
	...
	private JLabel lblMensajeMarca;
	...
	public void cargarListaClientes(){
		dataTable.DAOModelo.ConsultaSQL =  "SELECT idmodelo AS ID,DATE_FORMAT( fecha, '%d/%m/%Y' ) as Fecha, Nombre AS 'Marca',descripcion as 'Descripcion', activo as Vigente,idmarca FROM modelo";
		SincronizarTabla consulta = new SincronizarTabla(dataTable,lblEstado, internalToolBarMenu.getBtnActualizar(),jsBuscarCliente);
		consulta.execute();	
		SincronizarComboMarca cm = new SincronizarComboMarca(cbxMarca, lblMensajeMarca);
		cm.execute();
		modelo = new Modelo();	
			...
			internalToolBarMenu.getBtnGuardar().addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					if(verificarRequerimientos()){
						...
						modelo.setFecha(new Date(dcFecha.getDate().getTime()));
						modelo.setNombre(txtNombre.getText());
						modelo.setDescripcion(txtDescripcion.getText());
						modelo.setActivo(cbxActivo.isSelected());
						modelo.setIdmarca(((Marca)cbxMarca.getSelectedItem()).getIdmarca() );
						
						if(txtId.getText().isEmpty()){
							if(BDModelo.guardar(modelo)){
								JOptionPane.showMessageDialog(null, "Registro Guardado Satisfactoriamente");
							}
						}else{	
							modelo.setIdmodelo(Integer.parseInt(txtId.getText()));
							if(BDModelo.Modificar(modelo)){
				...
				private boolean verificarRequerimientos() {
					try{
						...
						return (((JTextField)dcFecha.getDateEditor().getUiComponent()).getForeground()==Color.BLACK) && !txtNombre.getText().isEmpty() && cbxMarca.getItemAt(cbxMarca.getSelectedIndex()).getIdmarca()!=-1;
					}catch(Exception ex){
					}
					return false;
				}
	...
	public JPanel getPanelGeneral() {
		if (panelGeneral == null) {
			...
			gbc_txtRuc.gridx = 2;
			gbc_txtRuc.gridy = 2;
			panelGeneral.add(gettxtRuc(), gbc_txtRuc);
			GridBagConstraints gbc_lblMensajeMarca = new GridBagConstraints();
			gbc_lblMensajeMarca.anchor = GridBagConstraints.WEST;
			gbc_lblMensajeMarca.insets = new Insets(0, 0, 5, 5);
			gbc_lblMensajeMarca.gridx = 3;
			gbc_lblMensajeMarca.gridy = 2;
			panelGeneral.add(getLblMensajeMarca(), gbc_lblMensajeMarca);
	...
	public JComboBox<Marca> gettxtRuc() {
		if (cbxMarca == null) {
			cbxMarca = new JComboBox<Marca>();
			cbxMarca.addFocusListener(new FocusAdapter() {
			...
			cbxMarca.addKeyListener(new KeyAdapter() {
				@Override
			...
			cbxMarca.setBorder(new SoftBevelBorder(BevelBorder.LOWERED, null, null, null, null));
			cbxMarca.setFont(App.fuentePrincipal);
		}
		return cbxMarca;
	}
		...
		if (dataTable == null) {
			...
			rowSelMod.addListSelectionListener(new ListSelectionListener() {
				public void valueChanged(ListSelectionEvent evt) {
					...
					txtDescripcion.setText(dataTable.getValueAt(dataTable.getSelectedRow(), 3).toString());
					cbxActivo.setSelected(Boolean.parseBoolean(dataTable.getValueAt(dataTable.getSelectedRow(),4).toString()));
					int idMarca = Integer.parseInt(dataTable.getValueAt(dataTable.getSelectedRow(), 5).toString());
										
					/***********Seleccionar el item*****************/		
					
					for ( int i=0 ;i<cbxMarca.getItemCount(); i++)
						if(cbxMarca.getItemAt(i).getIdmarca()==idMarca){
							cbxMarca.setSelectedItem( cbxMarca.getItemAt(i));
							break;
						}	
				}
			});
	...
	public JLabel getLblMensajeMarca() {
		if (lblMensajeMarca == null) {
			lblMensajeMarca = new JLabel("");
			lblMensajeMarca.setFont(new Font("Segoe UI", Font.PLAIN, 14));
		}
		return lblMensajeMarca;
	}
