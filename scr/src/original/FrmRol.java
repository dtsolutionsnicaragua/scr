...
import componente.VisorTreeRenderer;
...
import original.EditorTreeRender;
...
public class FrmRol extends JPanel {
	...
	public void cargarListaPermiso() {
		...
		for (Menu menuActual : DAOmenus) {
			JEditorMenu visorActual = new JEditorMenu();
		...
		VisorTreeRenderer vtr = new VisorTreeRenderer(new JEditorMenu());
		EditorTreeRender etr = new EditorTreeRender(tree, vtr);
		tree.setCellRenderer(vtr);
		tree.setCellEditor(etr);
		
	//	vtr.setLeafIcon(new ImageIcon(FrmRol.class.getResource("/icon/general/imgFormulario.png")));
	//	vtr.setClosedIcon(new ImageIcon(FrmRol.class.getResource("/icon/general/imgMenu.png")));
	//	vtr.setOpenIcon(new ImageIcon(FrmRol.class.getResource("/icon/general/imgMenu.png")));
	...
	private DefaultMutableTreeNode buscarPadre2(DefaultMutableTreeNode nodoActual, int indicePadre) {
		...
		while (e.hasMoreElements()) {
			DefaultMutableTreeNode node = (DefaultMutableTreeNode) e.nextElement();
			JEditorMenu jem = (JEditorMenu) node.getUserObject();
					...
					new ActionListener() {
						public void actionPerformed(ActionEvent arg0) {
							if (verificarRequerimientos()) {
								...
								rol.setFecha(new Date(dcFecha.getDate()
										.getTime()));
	...
	public JTree getJtPermiso() {
		if (tree == null) {
			...
			JEditorMenu jem = new JEditorMenu();
			...
			tree.getSelectionModel().setSelectionMode(TreeSelectionModel.CONTIGUOUS_TREE_SELECTION);  
			 ...
			 tree.setEditable(true);
		        tree.setDragEnabled(true);
			//tree.setCellRenderer(new VisorTreeRenderer());
			//tree.setCellEditor(new JComponentTreeCellEditor( ));
